...
const keyClass = computed(() =>
  cn(
    'cladd-shortcut__key relative shrink-0 font-semibold',
...
const iconClass = computed(() =>
  cn(shortcutIconSizes[d.value.size], d.value.iconClassName),
);
...
function tokenEntry(token: string): ShortcutEntry {
  ...
  if (['escape', 'esc'].includes(key))
    return { kind: 'text', padded, text: 'ESC' };
  if (['enter', 'return'].includes(key))
    return { icon: KeyboardReturnIcon, kind: 'icon', padded };
  ...
  if (key === 'left')
    return { icon: KeyboardArrowLeftIcon, kind: 'icon', padded };
  if (key === 'right') {
    return {
      icon: KeyboardArrowLeftIcon,
