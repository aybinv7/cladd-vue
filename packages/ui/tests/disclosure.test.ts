...
test("CollapsibleRoot starts closed and opens on trigger click", async () => {
  ...
  expect(byTestId(mounted.root, "trigger").getAttribute("aria-expanded")).toBe("false");
  ...
  expect(byTestId(mounted.root, "trigger").getAttribute("aria-expanded")).toBe("true");
  mounted.app.unmount();
});
...
test("CollapsibleRoot honors defaultOpen and keepMounted", () => {
  const mounted = mountTree(
    h(CollapsibleRoot, { defaultOpen: true }, () => [
      h(CollapsiblePanel, { "data-testid": "panel", keepMounted: true }, () => "content"),
    ]),
  );
  ...
  const mounted = mountTree(
    h(CollapsibleRoot, null, () => [
      ...
      h(
        ...
        {
          default: ({ open }: { open: boolean }) => (open ? "expanded" : "collapsed"),
        },
      ),
    ]),
  );
...
test("AccordionRoot keeps a single item open by default and closes it on re-toggle", async () => {
  const mounted = mountTree(
    h(AccordionRoot, { defaultValue: "one" }, () => [
      h(AccordionItem, { value: "one" }, () => [
        h(AccordionTrigger, null, () => h("button", { "data-testid": "trigger-one" }, "One")),
        h(AccordionPanel, { "data-testid": "panel-one" }, () => "One content"),
      ]),
      h(AccordionItem, { value: "two" }, () => [
        h(AccordionTrigger, null, () => h("button", { "data-testid": "trigger-two" }, "Two")),
        h(AccordionPanel, { "data-testid": "panel-two" }, () => "Two content"),
      ]),
    ]),
  );

  expect(mounted.root.querySelector('[data-testid="panel-one"]')).not.toBeNull();
  ...
  expect(mounted.root.querySelector('[data-testid="panel-two"]')).not.toBeNull();
