  ...
  return mountTree(
    h(Tabs, { defaultValue }, () => [
      h(TabsList, { "data-testid": "tabs-list" }, () => [
        h(Tab, { "data-testid": "tab-overview", value: "overview" }, () => "Overview"),
        h(Tab, { "data-testid": "tab-activity", value: "activity" }, () => "Activity"),
      ]),
      h(TabPanel, { "data-testid": "panel-overview", value: "overview" }, () => "Overview content"),
      h(TabPanel, { "data-testid": "panel-activity", value: "activity" }, () => "Activity content"),
    ]),
  );
}
...
test("selects the default tab and mounts only its panel", () => {
  ...
  expect(mounted.root.querySelector('[data-testid="panel-overview"]')).not.toBeNull();
  expect(mounted.root.querySelector('[data-testid="panel-activity"]')).toBeNull();
  mounted.app.unmount();
});
...
test("switches the selected tab and panel on click", async () => {
  ...
  expect(byTestId(mounted.root, "tab-activity").getAttribute("aria-selected")).toBe("true");
  expect(byTestId(mounted.root, "tab-overview").getAttribute("aria-selected")).toBe("false");
  expect(mounted.root.querySelector('[data-testid="panel-activity"]')?.textContent).toBe(
    "Activity content",
  );
  expect(mounted.root.querySelector('[data-testid="panel-overview"]')).toBeNull();
  mounted.app.unmount();
});
...
test("moves selection with arrow keys and wraps at the ends", async () => {
  ...
  list.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "ArrowRight" }));
  await nextTick();
  expect(byTestId(mounted.root, "tab-activity").getAttribute("aria-selected")).toBe("true");

  list.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "ArrowRight" }));
  await nextTick();
  expect(byTestId(mounted.root, "tab-overview").getAttribute("aria-selected")).toBe("true");
  mounted.app.unmount();
});
...
test("keeps an inactive panel mounted (but hidden) when keepMounted is set", () => {
  const mounted = mountTree(
    h(Tabs, { defaultValue: "overview" }, () => [
      h(TabPanel, { "data-testid": "panel-overview", value: "overview" }, () => "Overview"),
      h(
        TabPanel,
        { "data-testid": "panel-activity", keepMounted: true, value: "activity" },
        () => "Activity",
      ),
    ]),
  );
  ...
  const harness = defineComponent({
    setup() {
      return () =>
        h(
          Tabs,
          { "onUpdate:value": (next: string) => (value.value = next), value: value.value },
          () => [
            h(TabsList, () => [
              h(Tab, { "data-testid": "tab-overview", value: "overview" }, () => "Overview"),
              h(Tab, { "data-testid": "tab-activity", value: "activity" }, () => "Activity"),
